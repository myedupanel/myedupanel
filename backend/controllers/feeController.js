...
const calculateLateFees = async (req, res) => { 
     try {
        ...
        // Mongoose aggregation update ko Prisma $executeRaw se replace kiya hai
        if (result.count > 0) {
...
const getProcessingPayments = async (req, res) => { 
     try {
        ...
        if (search) {
           whereClause.OR = [
               { student: { // Student name par search
                   OR: [
                     { first_name: { contains: search, mode: 'insensitive' } },
                     { father_name: { contains: search, mode: 'insensitive' } },
                     { last_name: { contains: search, mode: 'insensitive' } },
               ...
               { chequeNumber: { contains: search, mode: 'insensitive' } } // Cheque number par search
           ];
        }
...
const getEditedRecords = async (req, res) => { 
     try {
        ...
        const records = await prisma.feeRecord.findMany({
            ...
            orderBy: { id: 'desc' }, // updatedAt hamare schema mein nahi hai
            take: parseInt(limit, 10),
            skip: skip
        });
...
const assignAndCollectFee = async (req, res) => {
  ...
  const {
    studentId, // Yeh frontend se Mongoose ID string hai -> Prisma 'studentid' (Int)
    templateId, // Yeh frontend se Mongoose ID string hai -> Prisma 'id' (Int)
    dueDate,
    amountPaid, 
    paymentMode,
    paymentDate,
  ...
  const schoolId = req.user.schoolId; // Yeh String hai (sahih hai)
  const collectedByUserId = req.user.id; // Yeh Int hai (sahih hai)
  ...
  // 3. Start Database Transaction (Prisma style)
  console.log("[assignAndCollectFee] Prisma transaction starting.");
  
  let savedFeeRecord;
  let savedTransaction = null;
  let studentName, studentClassName, templateName; // Socket ke liye

  try {
    const result = await prisma.$transaction(async (tx) => {
      // 4. Fetch necessary documents
      ...
      const foundClassId = student.class.classid; // Prisma relation se mila
      ...
      // 6. Create Transaction *if* paying now
      if (isPayingNow) {
        ...
        savedTransaction = await tx.transaction.create({
          data: {
            ...
            paymentMode: paymentMode, // 'mode' ke bajaye 'paymentMode'
            status: transactionStatus,
            collectedById: collectedByUserId,
            notes,
            chequeNumber: (paymentMode === 'Cheque') ? notes : null
            // bankName schema mein hai, par Mongoose code use 'N/A' daal raha tha
          }
        });
        console.log(`[assignAndCollectFee] Transaction created: ${savedTransaction.id}, Status: ${transactionStatus}`);
      }
      
      return { savedFeeRecord, savedTransaction }; // Transaction se return karein
    }); // Prisma transaction yahaan khatam
      ...
      if (result.savedTransaction) {
        ...
        if (result.savedTransaction.status === 'Success') {
          io.emit('new_transaction_feed', {
            name: studentName || 'A Student',
            amount: result.savedTransaction.amountPaid
          });
        }
      }
    } else {
        console.warn("[assignAndCollectFee] Socket.IO instance not found.");
    }
  ...
  // Prisma transaction mein 'finally' block ki zaroorat nahi hoti
};
...
const updateFeeTemplate = async (req, res) => {
  ...
  try {
    const updatedTemplate = await prisma.feeTemplate.update({
        where: { id: templateIdInt, schoolId: schoolId }, // Check schoolId
        data: {
            name: name.trim(),
            description,
            items,
    ...
    if (req.io) {
        req.io.emit('fee_template_updated', updatedTemplate);
    }
    ...
    console.error("Error in updateFeeTemplate:", error);
     if (error.code === 'P2025') { // "Record not found" error
       return res.status(404).json({ message: 'Fee template not found or you do not have permission.' });
     }
...
const deleteFeeTemplate = async (req, res) => {
  ...
  try {
    ...
    if (recordInUse) {
      return res.status(400).json({ message: 'Failed to delete template. It is already assigned to one or more students.' });
    }
    ...
    if (req.io) {
        req.io.emit('fee_template_deleted', { id: templateIdInt });
    }
...
// 14. Get Sample Sheet (Updated ID example)
const getSampleSheet = async (req, res) => { 
     try {
        const sampleData = [
          { 
              FeeRecordID_ToUpdate: 'Enter_Prismaid_Here (e.g., 12)', // Updated text
              AmountPaid_Partially: 500,
              PaymentDate: 'YYYY-MM-DD (e.g., 2025-10-26)',
              PaymentMode: 'Cash | Cheque | NEFT | UPI etc.',
              Notes: 'Optional notes (e.g., Received part payment)',
              ChequeNumber: 'If PaymentMode is Cheque',
              BankName: 'If PaymentMode is Cheque/NEFT/RTGS'
          },
          // ... (baaki sample data)
        ];
        
        // ... (Baaki 'xlsx' code same rahega) ...
...
const updateExistingRecords = async (req, res) => { 
    ...
    try {
        ...
        return res.status(400).json({ message: 'Could not parse the Excel file.' });
    }

    const sheetName = workbook.SheetNames[0];
    if (!sheetName) return res.status(400).json({ message: 'No sheets found.' });
    const data = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName]);
    if (data.length === 0) return res.status(400).json({ message: 'Sheet is empty.' });

    let successCount = 0;
    let errorCount = 0;
    const errors = [];
    const updatedFeeRecordIds = new Set();
    ...
    for (const [index, row] of data.entries()) {
        ...
        const mode = row.PaymentMode?.trim();
        const notes = row.Notes;
        const chequeNumber = row.ChequeNumber;
        const bankName = row.BankName;

        if (isNaN(feeRecordIdInt)) {
          errorCount++; errors.push(`Row ${index + 2}: Invalid or missing FeeRecordID.`); continue;
        }
        const amountPaid = Number(amountPaidInput);
        if (isNaN(amountPaid) || amountPaid <= 0) {
          errorCount++; errors.push(`Row ${index + 2} (ID: ${feeRecordIdInt}): Invalid or missing AmountPaid.`); continue;
        }
         if (!mode) {
           errorCount++; errors.push(`Row ${index + 2} (ID: ${feeRecordIdInt}): Missing PaymentMode.`); continue;
        }
        const paymentDate = paymentDateInput instanceof Date ? paymentDateInput : new Date();

        try {
            // Har row ke liye ek alag transaction
            await prisma.$transaction(async (tx) => {
                const feeRecord = await tx.feeRecord.findUnique({ 
                    where: { id: feeRecordIdInt, schoolId } 
                });
                if (!feeRecord) {
                  throw new Error(`Fee Record ${feeRecordIdInt} not found or doesn't belong to this school.`);
                }
                if (amountPaid > feeRecord.balanceDue) {
                  throw new Error(`Amount ${amountPaid} exceeds balance ${feeRecord.balanceDue}.`);
                }
                ...
                await tx.transaction.create({
                    data: {
                        ...
                        chequeNumber, 
                        bankName,
                        receiptId: `TXN-${Date.now()}-${index}` // Unique receipt ID
                    }
                });
    ...
    if (req.io && updatedFeeRecordIds.size > 0) {
        console.log(`[Import] Emitting socket events for ${updatedFeeRecordIds.size} updated records.`);
        req.io.emit('updateDashboard');
        req.io.emit('fee_records_updated');
    }

    res.status(200).json({
        message: `Import complete. ${successCount} payments recorded. ${errorCount} rows failed.`,
        errors: errors
    });
};
...
const exportDetailReport = async (req, res) => { 
     try {
        ...
        const transactions = await prisma.transaction.findMany({
            where: query,
            include: {
                student: { select: { first_name: true, father_name: true, last_name: true, roll_number: true, class: { select: { class_name: true }} } },
                template: { select: { name: true } },
        ...
        const dataForSheet = transactions.map(tx => ({
            ...
            'Student ID': tx.student?.roll_number || 'N/A', // Mongoose 'studentId' (string) ko 'roll_number' se replace kiya
        ...
        if (dataForSheet.length === 0) {
             return res.status(404).json({ message: 'No transactions found matching filters.' });
        }
        
        // ... (Baaki 'xlsx' code same rahega) ...
        const wb = xlsx.utils.book_new();
        const ws = xlsx.utils.json_to_sheet(dataForSheet);
        const columnWidths = [ {wch: 25}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 20}, {wch: 30}, {wch: 15}, {wch: 20} ];
        ws['!cols'] = columnWidths;
        xlsx.utils.book_append_sheet(wb, ws, "Detailed Fee Report");
        const buffer = xlsx.write(wb, { type: 'buffer', bookType: 'xlsx' });

        res.setHeader('Content-Disposition', 'attachment; filename="Detailed_Fee_Report.xlsx"');
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.send(buffer);
      } catch (error) {
...
const getPaidTransactions = async (req, res) => { 
     try {
        ...
        const allPaidTransactions = await prisma.transaction.findMany({
          where: {
            schoolId: schoolId, 
            studentId: studentIdInt,
            status: 'Success'
          ...
          include: {
             template: { select: { name: true } },
             feeRecord: { select: { isDeposit: true } }
          },
          orderBy: { paymentDate: 'desc' }
        });

        const deposits = allPaidTransactions.filter(tx => tx.feeRecord?.isDeposit === true);
        const paidRecords = allPaidTransactions.filter(tx => !tx.feeRecord || tx.feeRecord?.isDeposit !== true);
...
const getPaymentHistory = async (req, res) => { 
     try {
        ...
        const historyRecords = await prisma.transaction.findMany({
          where: {
            schoolId: schoolId,
            studentId: studentIdInt, 
          },
...
const collectManualFee = async (req, res) => { 
    ...
    const schoolId = req.user.schoolId;
    const collectedByUserId = req.user.id; // Prisma ID (Int)
    ...
    try {
        // Prisma transaction shuru
        const result = await prisma.$transaction(async (tx) => {
            const feeRecord = await tx.feeRecord.findUnique({
                where: { id: feeRecordIdInt, schoolId }
            });
            if (!feeRecord) {
                throw new Error('Fee Record not found or does not belong to this school.');
            }
            if (amountPaid > feeRecord.balanceDue + 0.01) { // Tolerance
            ...
            if (newTransaction.status === 'Success') {
                ...
                console.log(`[Manual Collect] Cheque received for FeeRecord: ${feeRecord.id}.`);
            }
            
            return { newTransaction, updatedFeeRecord };
        }); // Prisma transaction khatam

        console.log(`[Manual Collect] Transaction committed successfully.`);

        // Ab poora data fetch karein socket aur response ke liye
        ...
        console.error('Error collecting manual fee:', error);
...
const getTransactionById = async (req, res) => { 
     try {
        const transactionIdInt = parseInt(req.params.id);
        const schoolId = req.user.schoolId;
        ...
        const transaction = await prisma.transaction.findUnique({
          where: { id: transactionIdInt, schoolId: schoolId },
          include: {
            student: { include: { class: { select: { class_name: true } } } },
        ...
        const feeRecord = await prisma.feeRecord.findUnique({
            ...
            select: { amount: true, discount: true, lateFine: true }
        });
        
        const schoolInfo = await prisma.school.findUnique({
            where: { id: schoolId },
            select: { name: true, address: true, logo: true, session: true }
        });
        ...
        const receiptData = {
            ...
            templateItems: transaction.template?.items, // Naya add kiya (Mongoose mein tha)
        ...
        // Extra data jo frontend ko nahi chahiye, delete kar dein
        delete receiptData.student;
        delete receiptData.template;
        delete receiptData.collectedBy;
...
const getClasswiseReport = async (req, res) => { 
    try {
        ...
        // Kadam 1: Classes aur unka student count fetch karein
        ...
        // Kadam 2: Har class ki successful collection fetch karein
        ...
        // Kadam 3: Dono data ko merge karein
        const report = classes.map(cls => {
            const collection = reportData.find(r => r.classId === cls.classid);
            return {
                classId: cls.classid,
                className: cls.class_name,
                totalCollection: collection?._sum.amountPaid || 0,
...
const getStudentReportByClass = async (req, res) => { 
     try {
        ...
        // Kadam 1: Uss class ke saare students fetch karein
        const students = await prisma.students.findMany({
            where: { schoolId, classId: classIdInt },
            orderBy: { first_name: 'asc' }
        });

        if (students.length === 0) {
            return res.status(200).json([]); // Khaali array bhejein
        }

        const studentIds = students.map(s => s.studentid);

        // Kadam 2: Un students ka total paid amount fetch karein
        const paidData = await prisma.transaction.groupBy({
            by: ['studentId'],
            where: { 
                schoolId, 
                classId: classIdInt, 
        ...
        // Kadam 3: Un students ka total due/balance fetch karein
        const feeData = await prisma.feeRecord.groupBy({
            by: ['studentId'],
            where: {
                schoolId,
                classId: classIdInt,
        ...
        // Kadam 4: Saare data ko merge karein
        const report = students.map(student => {
            const paid = paidData.find(p => p.studentId === student.studentid);
            const fee = feeData.find(f => f.studentId === student.studentid);
            
            return {
                studentId: student.studentid,
                studentName: getFullName(student),
                studentRegId: student.roll_number || 'N/A', // Mongoose 'studentId' (string) ko 'roll_number' se replace kiya
                totalPaid: paid?._sum.amountPaid || 0,
                totalDue: fee?._sum.amount || 0,
                totalDiscount: fee?._sum.discount || 0,
                totalBalance: fee?._sum.balanceDue || 0
            }
        });
        
        res.status(200).json(report);
      } catch (error) {
        console.error("Error fetching student-wise report by class:", error);
         res.status(500).send("Server Error");
      }
};
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingTransaction) {
                ...
                // Ek error throw karein taaki transaction rollback ho
            ...
            newTransaction = await tx.transaction.create({
                data: {
                    ...
                    // collectedById yahaan null rahega
                }
            });
        ...
        // Socket ke liye student ka naam fetch karein
        ...
        res.status(200).json({ status: 'ok' }); // Razorpay ko OK bhejein

    } catch (error) {
        console.error('Webhook Error: Critical error processing payment:', payment?.id, error);
        // Agar "Transaction already processed" error hai, toh bhi Razorpay ko OK bhejein
        if (error.message === 'Transaction already processed') {
            return res.status(200).json({ message: 'Transaction already processed' });
        }
        // Asli error ke liye 500 bhejein
...
const getTransactions = async (req, res) => { 
    try {
        ...
        const {
            ...
            studentId // Yeh Mongoose string ID tha
        } = req.query;
        ...
        const formattedTransactions = transactions.map(tx => ({
            ...tx,
            studentId: { // Mongoose .populate('studentId', 'name studentId') jaisa object banayein
