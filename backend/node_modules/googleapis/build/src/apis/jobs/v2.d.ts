...
export declare namespace jobs_v2 {
    ...
    export interface Schema$Company {
        ...
        /**
         * Optional. The unique company identifier provided by the client to identify an employer for billing purposes. Recommended practice is to use the distributor_companyid. Defaults to same value as distributor_companyid when a value is not provided.
         */
    ...
    /**
     * A resource that represents an external Google identifier for a company, for example, a Google+ business page or a Google Maps business page. For unsupported types, use `unknown_typeid`.
     */
    ...
    export interface Schema$GetHistogramRequest {
        ...
        /**
         * Meta information, such as `userid`, collected from the job searcher or other entity conducting a job search, is used to improve the service's search quality. Users determine identifier values, which must be unique and consist.
         */
    ...
    export interface Schema$Job {
        ...
        companyDisplayName?: string | null;
        /**
         * Optional but one of company_name or distributor_companyid must be provided. The resource name of the company listing the job, such as /companies/foo. This field takes precedence over the distributor-assigned company identifier, distributor_companyid.
         */
        ...
        description?: string | null;
        /**
         * Optional but one of company_name or distributor_companyid must be provided. A unique company identifier used by job distributors to identify an employer's company entity. company_name takes precedence over this field, and is the recommended field to use to identify companies. The maximum number of allowed characters is 255.
         */
        distributorCompanyId?: string | null;
        /**
        ...
        level?: string | null;
        /**
         * Optional but strongly recommended for the best service experience. Location(s) where the emploeyer is looking to hire for this job posting. Specifying the full street address(es) of the hiring location enables better API results, especially job searches by commute time. At most 50 locations are allowed for best search performance. If a job has more locations, it is suggested to split it into multiple jobs with unique requisitionids (e.g. 'ReqA' becomes 'ReqA-1', 'ReqA-2', etc.) as multiple jobs with the same requisitionid, company_name and language_code are not allowed. If the original requisitionid must be preserved, a custom field should be used for storage. It is also suggested to group the locations that close to each other in the same job for better search experience. The maximum number of allowed characters is 500.
         */
        locations?: string[] | null;
        /**
         * Required during job update. Resource name assigned to a job by the API, for example, "/jobs/foo". Use of this field in job queries and API calls is preferred over the use of requisitionid since this value is unique.
         */
        name?: string | null;
        /**
    ...
    export class Resource$Jobs {
         ...
         *     // Optional. If set to `true`, only Job.name, Job.requisitionid and Job.language_code will be returned. A typical use case is to synchronize job repositories. Defaults to false.
         *     idsOnly: 'placeholder-value',
         *     // Optional. The maximum number of jobs to be returned per page of results. If ids_only is set to true, the maximum allowed page size is 1000. Otherwise, the maximum allowed page size is 100. Default is 100 if empty or a number < 1 is specified.
         *     pageSize: 'placeholder-value',
         *     // Optional. The starting point of a query result.
