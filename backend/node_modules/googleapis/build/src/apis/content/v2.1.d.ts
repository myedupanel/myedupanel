...
export declare namespace content_v2_1 {
    ...
    export interface Schema$BestSellers {
        /**
         * Google product category ID to calculate the ranking for, represented in [Google's product taxonomy](https://support.google.com/merchants/answer/6324436). If a `WHERE` condition on `best_sellers.categoryid` is not specified in the query, rankings for all top-level categories are returned.
         */
    ...
    export interface Schema$CompetitiveVisibility {
        ...
        /**
         * Google product category ID to calculate the report for, represented in [Google's product taxonomy](https://support.google.com/merchants/answer/6324436). Required in the `SELECT` clause. A `WHERE` condition on `competitive_visibility.categoryid` is required in the query.
         */
        categoryId?: string | null;
        /**
        ...
        /**
         * Position of the domain in the top merchants ranking for the selected keys (`date`, `categoryid`, `country_code`, `listing_type`) based on impressions. 1 is the highest. Available only in `CompetitiveVisibilityTopMerchantView` and `CompetitiveVisibilityCompetitorView`. Cannot be filtered on in the 'WHERE' clause.
         */
        rank?: string | null;
        /**
    ...
    /**
     * Represents how many items are in the shipment for the given shipmentid and line_itemid.
     */
    ...
    export interface Schema$OrderTrackingSignalShippingInfo {
        /**
         * The time when the shipment was actually delivered. Include the year and timezone string, if available. This field is required, if one of the following fields is absent: trackingid or carrier_name.
         */
        ...
        /**
         * The earliest delivery promised time. Include the year and timezone string, if available. This field is required, if one of the following fields is absent: trackingid or carrier_name.
         */
        earliestDeliveryPromiseTime?: Schema$DateTime;
        /**
         * The latest delivery promised time. Include the year and timezone string, if available. This field is required, if one of the following fields is absent: trackingid or carrier_name.
         */
    ...
    /**
     * Dimensions according to which metrics are segmented in the response. Values of product dimensions, such as `offerid`, reflect the state of a product at the time of the corresponding event, for example, impression or order. Segment fields cannot be selected in queries without also selecting at least one metric field. Values are only set for dimensions requested explicitly in the request's search query.
     */
    ...
    /**
     * Specifications related to the `Checkout` URL. The `UriTemplate` is of the form `https://www.mystore.com/checkout?itemid={id\}` where `{id\}` will be automatically replaced with data from the merchant account with this attribute [offerid](https://developers.google.com/shopping-content/reference/rest/v2.1/products#Product.FIELDS.offerid)
     */
