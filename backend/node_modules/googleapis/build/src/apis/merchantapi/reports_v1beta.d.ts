...
export declare namespace merchantapi_reports_v1beta {
    ...
    export interface Schema$BestSellersBrandView {
        ...
        /**
         * Google product category ID to calculate the ranking for, represented in [Google's product taxonomy](https://support.google.com/merchants/answer/6324436). Required in the `SELECT` clause. If a `WHERE` condition on `report_categoryid` is not specified in the query, rankings for all top-level categories are returned.
         */
    ...
    export interface Schema$CompetitiveVisibilityBenchmarkView {
        ...
        date?: Schema$Date;
        /**
         * Google product category ID to calculate the report for, represented in [Google's product taxonomy](https://support.google.com/merchants/answer/6324436). Required in the `SELECT` clause. A condition on `report_categoryid` is required in the `WHERE` clause.
         */
        reportCategoryId?: string | null;
        /**
    ...
    export interface Schema$CompetitiveVisibilityCompetitorView {
        ...
        /**
         * Position of the domain in the similar businesses ranking for the selected keys (`date`, `report_categoryid`, `report_country_code`, `traffic_source`) based on impressions. 1 is the highest. Cannot be filtered on in the 'WHERE' clause.
         */
        rank?: string | null;
        /**
    ...
    export interface Schema$CompetitiveVisibilityTopMerchantView {
        ...
        pageOverlapRate?: number | null;
        /**
         * Position of the domain in the top merchants ranking for the selected keys (`date`, `report_categoryid`, `report_country_code`, `traffic_source`) based on impressions. 1 is the highest. Cannot be filtered on in the 'WHERE' clause.
         */
        rank?: string | null;
        /**
         * Google product category ID to calculate the report for, represented in [Google's product taxonomy](https://support.google.com/merchants/answer/6324436). Required in the `SELECT` clause. A condition on `report_categoryid` is required in the `WHERE` clause.
    ...
    /**
     * Fields available for query in `product_performance_view` table. Product performance data for your account, including performance metrics (for example, `clicks`) and dimensions according to which performance metrics are segmented (for example, `offerid`). Values of product dimensions, such as `offerid`, reflect the state of a product at the time of the impression. Segment fields cannot be selected in queries without also selecting at least one metric field. Values are only set for fields requested explicitly in the request's search query.
     */
    ...
    export interface Schema$ProductStatusChangeMessage {
        ...
        /**
         * The account that manages the merchant's account. can be the same as merchant id if it is standalone account. Format : `accounts/{service_providerid\}`
         */
