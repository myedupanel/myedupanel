...
export declare namespace runtimeconfig_v1beta1 {
    ...
    export class Resource$Projects$Configs {
         ...
         *     // An optional but recommended unique `requestid`. If the server receives two `create()` requests with the same `requestid`, then the second request will be ignored and the first resource created and stored in the backend is returned. Empty `requestid` fields are ignored. It is responsibility of the client to ensure uniqueness of the `requestid` strings. `requestid` strings are limited to 64 characters.
