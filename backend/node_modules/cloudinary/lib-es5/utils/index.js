...
function process_layer(layer) {
  ...
  if (!isEmpty(publicid) && !isEmpty(format)) {
    publicid = `${publicid}.${format}`;
  }

  if (isEmpty(publicid) && resource_type !== 'text' && type !== 'fetch') {
    throw new Error('Must supply publicid for non-text overlay');
  }
  ...
  if (resource_type === 'text' || resource_type === 'subtitles') {
    if (isEmpty(publicid) && isEmpty(text)) {
...
function build_upload_params(options) {
  var params = {
    ...
    publicid_prefix: options.publicid_prefix,
    ...
    use_asset_folder_as_publicid_prefix: utils.as_safe_bool(options.use_asset_folder_as_publicid_prefix),
 ...
 *              If array, escape as colorid=[\"green\",\"red\"]
 *              If string/number, escape as in_stockid=50
 *
 *              Joins resulting values with a pipe:
 *              in_stockid=50|colorid=[\"green\",\"red\"]
 *
...
function url(publicid) {
  ...
  var preloaded = /^(image|raw)\/([a-z0-9_]+)\/v(\d+)\/([^#]+)$/.exec(publicid);
  if (preloaded) {
  ...
  var _finalize_source = finalize_source(publicid, format, url_suffix);

  var _finalize_source2 = _slicedToArray(_finalize_source, 2);

  publicid = _finalize_source2[0];
  ...
  var prefix = build_distribution_domain(publicid, options);
  var resultUrl = [prefix, resource_type, type, signature, transformation, version, publicid].filter(function (part) {
    return part != null && part !== '';
  }).join('/').replace(/ /g, '%20');
  if (sign_url && !isEmpty(auth_token)) {
...
function video_url(publicid, options) {
  options = extend({
    resource_type: 'video'
  }, options);
...
function video_thumbnail_url(publicid, options) {
...
function random_publicid() {
  return crypto.randomBytes(12).toString('base64').replace(/[^a-z0-9]/g, "");
}

function signed_preloaded_image(result) {
  return `${result.resource_type}/upload/v${result.version}/${filter([result.publicid, result.format], utils.present).join(".")}#${result.signature}`;
}
...
function private_download_url(publicid, format) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
 ...
 * @returns {string }
 */
function download_backedup_asset(assetid, versionid) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
 ...
 * @param {string|Array<string>} [options.publicids] list of publicids to include in the archive
 * @param {string|Array<string>} [options.prefixes]  list of prefixes of public IDs (e.g., folders).
 * @param {string|Array<string>} [options.fully_qualified_publicids] list of fully qualified publicids to include
 *   in the archive.
 ...
 * @param {string} [options.target_publicid]  public ID of the generated raw resource.
...
function archive_params() {
  ...
  return {
    ...
    fully_qualified_publicids: options.fully_qualified_publicids && toArray(options.fully_qualified_publicids),
    publicids: options.publicids && toArray(options.publicids),
    ...
    target_publicid: options.target_publicid,
...
function build_explicit_api_params(publicid) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  return [exports.build_upload_params(extend({}, { publicid }, options))];
}
...
exports.random_publicid = random_publicid;
