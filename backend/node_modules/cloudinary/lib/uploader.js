...
exports.upload_chunked_stream = function upload_chunked_stream(callback, options = {}) {
  ...
  options.x_unique_uploadid = utils.random_publicid();
  let params = build_upload_params(options);
...
exports.explicit = function explicit(publicid, callback, options = {}) {
  return call_api("explicit", callback, options, function () {
    return utils.build_explicit_api_params(publicid, options);
  });
};
...
exports.destroy = function destroy(publicid, callback, options = {}) {
...
exports.rename = function rename(from_publicid, to_publicid, callback, options = {}) {
  return call_api("rename", callback, options, function () {
    return [
      {
        timestamp: utils.timestamp(),
        type: options.type,
        from_publicid: from_publicid,
        to_publicid: to_publicid,
        overwrite: options.overwrite,
        invalidate: options.invalidate,
...
const TEXT_PARAMS = ["publicid", "font_family", "font_size", "font_color", "text_align", "font_weight", "font_style", "background", "opacity", "text_decoration", "font_hinting", "font_antialiasing"];
...
exports.explode = function explode(publicid, callback, options = {}) {
 ...
 *
 * @param {Array}          publicids           A list of public IDs (up to 1000) of assets uploaded to Cloudinary.
 *
 * @param {Function}        callback             Callback function
 *
 ...
 * @return {Object}
 */
exports.add_tag = function add_tag(tag, publicids = [], callback, options = {}) {
  const exclusive = utils.option_consume("exclusive", options);
  const command = exclusive ? "set_exclusive" : "add";
  return call_tags_api(tag, command, publicids, callback, options);
};
 ...
 * @return {Object}
 */
exports.remove_tag = function remove_tag(tag, publicids = [], callback, options = {}) {
  return call_tags_api(tag, "remove", publicids, callback, options);
};

exports.remove_all_tags = function remove_all_tags(publicids = [], callback, options = {}) {
  return call_tags_api(null, "remove_all", publicids, callback, options);
};

exports.replace_tag = function replace_tag(tag, publicids = [], callback, options = {}) {
  return call_tags_api(tag, "replace", publicids, callback, options);
};

function call_tags_api(tag, command, publicids = [], callback, options = {}) {
  return call_api("tags", callback, options, function () {
    let params = {
      timestamp: utils.timestamp(),
      publicids: utils.build_array(publicids),
      command: command,
      type: options.type
    };
    if (tag != null) {
...
exports.add_context = function add_context(context, publicids = [], callback, options = {}) {
  return call_context_api(context, 'add', publicids, callback, options);
};

exports.remove_all_context = function remove_all_context(publicids = [], callback, options = {}) {
  return call_context_api(null, 'remove_all', publicids, callback, options);
};

function call_context_api(context, command, publicids = [], callback, options = {}) {
...
function call_api(action, callback, options, get_params) {
  ...
  let boundary = utils.random_publicid();
...
function post(url, post_data, boundary, file, callback, options) {
  ...
  if (options.x_unique_uploadid != null) {
    headers['X-Unique-Upload-Id'] = options.x_unique_uploadid;
  }
 ...
 *
 * @param {Object}   metadata   A list of custom metadata fields (by externalid) and the values to assign to each
 * @param {Array}    publicids The public IDs of the resources to update
 * @param {Function} callback   Callback function
 * @param {Object}   options    Configuration options
 *
 * @return {Object}
 */
exports.update_metadata = function update_metadata(metadata, publicids, callback, options = {}) {
