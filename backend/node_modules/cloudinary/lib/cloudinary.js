...
exports.url = function url(publicid, options) {
...
/**
 * Creates an HTML video tag for the provided publicid
 ...
 *        poster options that may include a <tt>publicid</tt> key and
 ...
 */
exports.video = function video(publicid, options) {
  options = _.extend({}, options);
  ...
  if (video_options.hasOwnProperty('poster')) {
    if (_.isPlainObject(video_options.poster)) {
      if (video_options.poster.hasOwnProperty('publicid')) {
        video_options.poster = cloudinary.utils.url(video_options.poster.publicid, video_options.poster);
      } else {
        video_options.poster = cloudinary.utils.url(publicid, _.extend({}, cloudinary.utils.DEFAULT_POSTER_OPTIONS, video_options.poster));
      }
    }
  } else {
    video_options.poster = cloudinary.utils.url(publicid, _.extend({}, cloudinary.utils.DEFAULT_POSTER_OPTIONS, options));
  }
 ...
 * @return {string}
 */
exports.source = function source(publicid, options = {}) {
  let srcsetParam = cloudinary.utils.extend({}, options.srcset, cloudinary.config().srcset);
  let attributes = options.attributes || {};

  cloudinary.utils.extend(attributes, generateImageResponsiveAttributes(publicid, attributes, srcsetParam, options));
  if (!attributes.srcset) {
    attributes.srcset = cloudinary.url(publicid, options);
  }
 ...
 * );
 */
exports.picture = function picture(publicid, options = {}) {
  ...
  return "<picture>"
    + sources.map((source) => {
      ...
      return cloudinary.source(publicid, sourceOptions);
    }).join('')
    + cloudinary.image(publicid, options)
    + "</picture>";
};
