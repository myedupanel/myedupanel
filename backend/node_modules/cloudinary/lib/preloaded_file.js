...
class PreloadedFile {
  constructor(file_info) {
    let matches, publicid_and_format;
    ...
    publicid_and_format = PreloadedFile.split_format(this.filename);
    this.publicid = publicid_and_format[0];
    this.format = publicid_and_format[1];
  }
  ...
  static split_format(identifier) {
    let format, last_dot, publicid;
    last_dot = identifier.lastIndexOf(".");
    if (last_dot === -1) {
      return [identifier, null];
    }
    publicid = identifier.substr(0, last_dot);
