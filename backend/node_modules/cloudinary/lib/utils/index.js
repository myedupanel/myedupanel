...
function url(publicid, options = {}) {
  ...
  let preloaded = /^(image|raw)\/([a-z0-9_]+)\/v(\d+)\/([^#]+)$/.exec(publicid);
  if (preloaded) {
  ...
  [publicid, source_to_sign] = finalize_source(publicid, format, url_suffix);
  ...
  let prefix = build_distribution_domain(publicid, options);
  let resultUrl = [prefix, resource_type, type, signature, transformation, version, publicid].filter(function (part) {
    return (part != null) && part !== '';
  }).join('/').replace(/ /g, '%20');
  if (sign_url && !isEmpty(auth_token)) {
...
function private_download_url(publicid, format, options = {}) {
 ...
 * @returns {string }
 */
function download_backedup_asset(assetid, versionid, options = {}) {
  let params = exports.sign_request({
    timestamp: options.timestamp || exports.timestamp(),
    assetid: assetid,
    versionid: versionid
...
function build_explicit_api_params(publicid, options = {}) {
