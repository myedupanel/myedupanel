 ...
 */
function scaledUrl(publicid, width, transformation, options = {}) {
  ...
  return utils.url(publicid, configParams);
}
 ...
 * @return {*|Array}
 */
function getOrGenerateBreakpoints(publicid, srcset = {}, options = {}) {
  let breakpoints = [];
  if (srcset.useCache) {
    breakpoints = Cache.get(publicid, options);
    if (!breakpoints) {
      breakpoints = [];
    }
  } else {
...
function generateSrcsetAttribute(publicid, breakpoints, transformation, options) {
  ...
  return breakpoints.map(width => `${scaledUrl(publicid, width, transformation, options)} ${width}w`).join(', ');
}
